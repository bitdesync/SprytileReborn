...
class SprytileSceneSettings(bpy.types.PropertyGroup):
    ...
    def get_layer(self):
        ...
        layer_value = self.get("work_layer", 1)
        if layer_value not in index_value_lookup:
            layer_value = 1
        set_idx = index_value_lookup.index(layer_value)
...
class SprytileMaterialData(bpy.types.PropertyGroup):
    ...
    def set_expanded(self, value):
        do_rebuild = self.get('is_expanded', True) != value
...
def sprytile_load_handler(dummy):
    ...
    if not bpy.data.filepath:
        addon_prefs = sprytile_utils.get_addon_preferences(bpy.context)
        if addon_prefs:
