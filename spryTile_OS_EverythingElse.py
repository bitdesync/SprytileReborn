...
def get_flat_shader():
    global flat_shader
    if flat_shader is not None:
        return flat_shader

    shader_info = gpu.types.GPUShaderCreateInfo()
    iface = gpu.types.GPUStageInterfaceInfo("sprytile_flat_iface")
    iface.smooth('VEC4', "o_color")
    shader_info.vertex_in(0, 'VEC2', "i_position")
    shader_info.vertex_in(1, 'VEC4', "i_color")
    ...
    flat_shader = gpu.shader.create_from_info(shader_info)
    return flat_shader


def get_image_shader():
    global image_shader
    if image_shader is not None:
        return image_shader
    ...
    shader_info.vertex_in(0, 'VEC2', "i_position")
    shader_info.vertex_in(1, 'VEC4', "i_color")
    shader_info.vertex_in(2, 'VEC2', "i_uv")
    shader_info.vertex_out(iface)
    shader_info.sampler(0, 'FLOAT_2D', "u_image")
    shader_info.push_constant('MAT4', "u_modelViewProjectionMatrix")
    shader_info.push_constant('FLOAT', "u_correct")
    shader_info.fragment_out(0, 'VEC4', 'frag_color')
    shader_info.vertex_source(
        '''
        void main()
    ...
    image_shader = gpu.shader.create_from_info(shader_info)
    return image_shader
...
class VIEW3D_OP_SprytileGui(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if len(context.scene.sprytile_mats) < 1:
            return {'CANCELLED'}
        if VIEW3D_OP_SprytileGui.is_running:
        ...
        VIEW3D_OP_SprytileGui.active_instance = self
    ...
    def exit(self, context):
        ...
        VIEW3D_OP_SprytileGui.active_instance = None
    ...
    def handle_ui(self, context, event):
        ...
        if mouse_pt is not None and event.type in {'LEFTMOUSE', 'MIDDLEMOUSE', 'MOUSEMOVE', 'INBETWEEN_MOUSEMOVE'}:
            ...
            if event.type == 'LEFTMOUSE' and event.value == 'PRESS' and VIEW3D_OP_SprytileGui.is_selecting is False:
                addon_prefs = sprytile_utils.get_addon_preferences(context)
                move_mod_pressed = False
                tile_sel_move_key = addon_prefs.tile_sel_move_key if addon_prefs else 'Ctrl'
                if tile_sel_move_key == 'Alt':
                    move_mod_pressed = event.alt
                if tile_sel_move_key == 'Ctrl':
                    move_mod_pressed = event.ctrl
                if tile_sel_move_key == 'Shift':
    ...
    def setup_offscreen(self, context):
        ...
        if VIEW3D_OP_SprytileGui.offscreen:
            VIEW3D_OP_SprytileGui.texture = VIEW3D_OP_SprytileGui.get_offscreen_texture(VIEW3D_OP_SprytileGui.offscreen)
        else:
    ...
    @staticmethod
    def get_offscreen_texture(offscreen):
        if hasattr(offscreen, "texture_color"):
            return offscreen.texture_color
        return offscreen.color_texture
    ...
    def draw_offscreen(context):
        ...
        offscreen.bind()
        gpu.state.viewport_set(0, 0, tex_size[0], tex_size[1])
    ...
    def draw_preview_tile(context, region, rv3d, mvp_mat):
        ...
        addon_prefs = sprytile_utils.get_addon_preferences(context)
        preview_alpha = addon_prefs.preview_transparency if addon_prefs else 0.8
        sprytile_data = context.scene.sprytile_data
...
class VIEW3D_OP_SprytileGuiRestart(bpy.types.Operator):
    bl_idname = "sprytile.gui_restart"
    bl_label = "Show Sprytile Palette"
    bl_description = "Show or restart the Sprytile tile palette"

    force_restart: BoolProperty(
        name="Force Restart",
        default=True,
        options={'HIDDEN'}
    )
    ...
    def invoke(self, context, event):
        ...
        if len(context.scene.sprytile_mats) < 1:
            self.report({'WARNING'}, "No valid Sprytile materials")
            return {'CANCELLED'}

        if self.force_restart:
            active_instance = VIEW3D_OP_SprytileGui.active_instance
            if active_instance is not None:
                active_instance.exit(context)
            else:
                VIEW3D_OP_SprytileGui.handler_remove(self, context)
                VIEW3D_OP_SprytileGui.is_running = False
                VIEW3D_OP_SprytileGui.tile_ui_active = False
        ...
        context.scene.sprytile_ui.is_dirty = True
        result = bpy.ops.sprytile.gui_win('INVOKE_REGION_WIN')
        if context.area:
            context.area.tag_redraw()
        return result
