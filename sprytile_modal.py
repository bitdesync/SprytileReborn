...
try:
    ...
    from . import sprytile_uv, sprytile_utils, sprytile_preview
...
class SprytileEventSubscription:
    def __init__(self, stream, subscriber):
        self.stream = stream
        self.subscriber = subscriber

    def dispose(self):
        if self.subscriber in self.stream.subscribers:
            self.stream.subscribers.remove(self.subscriber)


class SprytileEventStream:
    def __init__(self):
        self.subscribers = []

    def filter(self, predicate):
        filtered = SprytileEventStream()

        def forward(value):
            if predicate(value):
                filtered.on_next(value)

        self.subscribe(on_next=forward)
        return filtered
    ...
    def subscribe(self, on_next=None, on_error=None, on_completed=None):
        ...
        return SprytileEventSubscription(self, subscriber)

    def on_next(self, value):
        for subscriber in tuple(self.subscribers):
            try:
                if subscriber["on_next"] is not None:
...
class VIEW3D_OP_SprytileModalTool(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        auto_adjust = addon_prefs.auto_adjust_viewport_shading if addon_prefs else True
        if auto_adjust and context.area:
        ...
        # Setup event stream for tool observers.
        self.rx_observer = SprytileEventStream()
        self.rx_source = self.rx_observer

        # Tools receive events from the stream.
        self.tools = {
        ...
        try:
            ...
            print("Unable to start Sprytile palette: {0}".format(err))
