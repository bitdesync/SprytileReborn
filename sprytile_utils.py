...
from os import path
try:
    from . import sprytile_modal, sprytile_preview, addon_updater_ops
except ImportError:
...
def get_addon_preferences(context):
    candidates = []
    for addon_id in (__package__, __name__.split('.')[0], 'ReSprytile', 'resprytile'):
        if addon_id and addon_id not in candidates:
...
class UTIL_OP_SprytileSetupViewport(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        eevee_settings = getattr(context.scene, "eevee", None)
        if eevee_settings is not None:
            if hasattr(eevee_settings, "taa_samples"):
                eevee_settings.taa_samples = 1
            if hasattr(eevee_settings, "use_taa_reprojection"):
                eevee_settings.use_taa_reprojection = False
